...
class TestSocketCreateStreamTableParser extends FunSuite{

  //good test create rabbitmq stream table

  //测试基本的建表逻辑 没有设定主键和水位
  test("good test create socket stream table: simple") {

    val expected = new SocketCreateStreamTableInfo("table1",
    ...
    val input =
      """
        ...
        |     salary double 10.2
        |) required( source = 'socket',
        |            dataType = 'json',
        |            keyDecoder = 'none',
        |            parallel = '10',
        |            castError = 'stop' )
        |  optional( ip = '192.168.0.1',
        |            port  = '9238',
        |            maxRetry = '3'
        |           )as table1
        |
  ...
  //测试基本的建表逻辑 设定主键
  test("good test create socket stream table: set primary key") {
  ...
  //测试基本的建表逻辑 设定水位
  test("good test create socket stream table: set waterMark") {
