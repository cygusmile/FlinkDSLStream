...
class TestHDFSCreateStreamTableParser extends FunSuite {

  //good test create hdfs stream table

  //测试基本的建表逻辑 没有设定主键和水位
  test("good test create hdfs stream table: simple") {

    val expected = new HDFSCreateStreamTableInfo("table1",
      fields = Seq(SourceField.constructCreateField(None, "userId", Some("uid"), Some(Long.box(0L)),FieldType.LONG),
  ...
  //测试基本的建表逻辑 设定主键
  test("good test create hdfs stream table: set primary key") {
  ...
  //测试基本的建表逻辑 设定水位
  test("good test create hdfs stream table: set waterMark") {

    val expected = new HDFSCreateStreamTableInfo("table1",
      fields = Seq(SourceField.constructCreateField(None, "userId", Some("uid"), Some(Long.box(0L)),FieldType.LONG),
        SourceField.constructCreateField(None, "age", None, None,FieldType.INT),
        SourceField.constructCreateField(Some(Array("info")), "address", Some("address"), Some(""),FieldType.STRING),
        SourceField.constructCreateField(Some(Array("info", "extend")), "timeStamp", Some("ts"), Some(Long.box(-1L)), FieldType.LONG),
        SourceField.constructCreateField(Some(Array("info", "extend")), "other", Some("arr"), Some(Array(1,2,3,4)),FieldType.INT_ARRAY)),
      key = Some("keyWord"),
